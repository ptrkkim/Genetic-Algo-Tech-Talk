...
export const tenSided = createCircularPolygon(10);
...
function createCircularPolygon(sides) {
  ...
  // const circular = [firstPoint, ...points];
  const circular = points;
  const lowestX = findLowest(circular, 'x');
  const lowestY = findLowest(circular, 'y');

  const offsetX = 10 - lowestX;
  const offsetY = 10 - lowestY;

  return circular.map(point => ({ x: point.x + offsetX, y: point.y + offsetY }));
}
